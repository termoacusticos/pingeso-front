...
import { getAllQuincallerias, saveQuincalleria, updateQuincalleria, deleteQuincalleria } from '$lib/repositories/quincalleria';
...
export const PUT: RequestHandler = async ({ request, platform, cookies }) => {
    ...
    const { id, quincalleriaData } = await request.json<{ id: number; quincalleriaData: Quincalleria}>();
