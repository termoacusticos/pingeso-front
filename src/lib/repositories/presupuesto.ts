...
export const savePresupuesto = async (db: D1Database, presupuesto: Presupuesto) => {
    return await db
        .prepare('INSERT INTO presupuesto (rut_usuario, fecha, data_json, nombre_cliente, rut_cliente) VALUES (?, ?, ?, ?, ?);')
        .bind(presupuesto.rut_usuario, presupuesto.fecha, presupuesto.data_json, presupuesto.nombre_cliente, presupuesto.rut_cliente)
        .run()
        .then(() => ok(true))
