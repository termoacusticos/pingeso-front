...
export const saveVentana = async (db: D1Database, ventana: Ventana) => {
    return await db
        .prepare('INSERT INTO ventana (descripcion, alto, ancho, minimo, maximo, id_opcion) VALUES (?, ?, ?, ?, ?, ?);')
        .bind(ventana.descripcion, ventana.alto, ventana.ancho, ventana.minimo, ventana.maximo, ventana.id_opcion)
        .run()
        .then(() => ok(true))
