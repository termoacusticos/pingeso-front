<script lang="ts">
  import { materialOptions, colorOptions, itemOptions, tipoOptions, cantidadOptions, altoOptions, anchoOptions } from '$lib/store'
	...
	let { ventana = $bindable(), id, option_index, mostrar_eliminar, eliminarVentana }: Props = $props();
...
<tr class="border">
  ...
  <!-- Item Selector -->
  <td class="px-2 py-2  ">
    <input type="text" bind:value={ventana.item} class="p-2 border rounded-md" placeholder="Ingrese item">
  </td>
  ...
  <!-- Cantidad Input -->
  <td class="px-2 py-2 ">
    <input type="number" bind:value={ventana.cantidad} min="1" class="p-2 border rounded-md w-full">
  </td>
